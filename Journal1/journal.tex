...
However, based on research such as \cite{HARINI}, indicates that cyber faults, can cause a
...
this, we present an approach in modeling the behavior of a system using models and Markov chains.
...
Orginally published in 1986, his algorithm provides a robust election 
...
Since Garcia-Molina's orginal publication, there has been a large body of
...
accross all work: nodes arrive at a consensus of a single peer who coordinates
...
process. The leader, will periodically search for
...
Each core functionality of the system is implemented within a module which is
...
FREEDM center targeting lower cost, less powerful ARM boards, with less
...
Note that there is a resend function is periodically called to attempt to redeliver
...
``Lost'' messages are dropped on the reciever side. Code was inserted in the
...
they have not drifted noticeably from their last synchronization. Additionally,
...
assumption, and the Markov Chains which models those scenarios needed to as
well.
...
the use of the program SharpE \cite{SHARPE}\cite{SHARPE2} made by Dr. Kishor
Trivedi's group at Duke University, and partially for the ease of the mathematics
involved in the continuous time Markov chain. Events are modeled in the chain
...
Correlation was calculated from the collected data: a correlation coefficient of .8184 was 
found for the 128ms resend case and a factor of .8754 was found for the 64ms resend case.
