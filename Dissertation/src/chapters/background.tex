...
In a matrix $P$, the value of $P_{ij}$ represents the probability of the transition from $w_i$ to $w_j$.
...
Where $P_{i}$ corresponds to the probability of observing state $w_i$.
\end{pdef}
...
The steady state is a state distribution vector that describes the probability a random observation of a long-running process will observe some state $w_i$.
...
$B$ then is a $|W|\times|Y|$ matrix where $B_ij = \Pr(y_j|W_i)$, the probability of observing $y_j$ given the system is in some hidden state $s_i$.
