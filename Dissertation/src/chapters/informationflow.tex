...
With a mapping of the probability of transitioning between worlds, the Kripke Model itself becomes a Markov Chain\cite{kripkemarkov1}\cite{kripkemarkov2}.
Since the Kripke model is complete representation of the system being modeled, each world must be memoryless with respect to any world that has an $R$ relation onto that world ($w_xRw_y$).
...
The complete set of wffs and beliefs available in a particular domain may not be necessary to construct the desired model nor may it be the state the model uses. Let $Im(D_i, w)$ define an image function for a particular domain $i$, and world $w$.
An image function is an observation of the state of the system from the view of a domain $D_i$.
The image function produces an observation suitable for use in a Markov chain or \ac{HMM}.
If an image function is non-hiding it can be used in a Markov chain.
Otherwise, it is hiding, and must be used in conjunction with a \ac{HMM}.

Let $\Pr(w_{i}Rw_{i+1})$ be the probability of transitioning from world $w_i$ to world $w_{i+1}$.
Let $\Pr(w_{i}Rw{i+1} \wedge w_{i+1}R... \wedge ...RW_{n})$ be the probability of following a sequence of worlds from some world $w_i$ to some world $w_n$.
There may be multiple sequences that lead from world $w_i$ to world $w_n$.
Let $\Pr(I_1 \rightarrow I_2)$ be the probability of transitioning from $I_1$ to $I_2$.
Let $W_1$ and $W_2$ be the set of worlds that produce $I_1$ and $I_2$, respectively.
Let $W_C$ be the set of worlds where an image of the system would be taken.
We restrict $W_1$ and $W_2$ to be a subset of $W_C$.
Let $W_1 = \{ w_x | Im(D_i, w_x) = I_1 \}$ and $W_2 = \{ w_x | Im(D_i, w_x) = I_2 \}$.
Note that $W_1 \subset W_C$ and $W_2 \subset W_C$.
We obligate that $I_1 \neq I_2 \rightarrow W_1 \cap W_2 = \emptyset$.
$I_2$ is a successor to $I_1$ in a Markov chain if there is a sequence of worlds for every $w_x \in W_1$ that leads to a $w_y \in W_2$.
Let $\Pr(w_x \rightarrow w_y)$ be the probability of going through any sequence $\Pr(w_{x}Rw{x+1} \wedge w_{x+1}R... \wedge ...RW_{y})$ where $w_x \in W_1$ and $w_y \in W_2$ and no world in that sequence except $w_x$ and $w_y$ are in $W_C$.

\begin{pdef}
    An image $I_1$ is non-hiding if, for every $I_2$ that is a successor to $I_1$, for every world $w_x \in W_1$ and every $w_y \in W_2$ that can be reached from $w_x$ before any other element of $W_C$, $\Pr(w_x \rightarrow w_y) = \Pr(w_x' \rightarrow w_y')$, where $w_x'$ and $w_y'$ are another pair of worlds that fulfill the same requirements for $w_x$ and $w_y$.
\end{pdef}

An image is hiding if the imaging function discards (or cannot determine) some part of the state necessary to produce a time-homogeneous Markov chain.

\begin{thm}
A Markov chain is non-hidden if every image used in the chain is non-hiding.
\end{thm}

\begin{proof}
    Proof: A Markov chain is not hidden if the observation and state match.
    If every image used in a Markov chain is non-hiding, every observation matches.
\end{proof}

\begin{thm}
    Any Markov chain constructed using an imaging function $Im()$ on a Kripke model is memoryless.
\end{thm}

\begin{proof}
    Proof: Let $I_1$ and $I_2$ be a pair of images, where $I_2$ is an immediate successor of $I_1$ in a Markov chain.
\begin{case}
    The Markov chain is non-hidden.
\end{case}

Since the Markov chain is non-hidden the probability of transitioning from any world in $W_1$ to any world in $W_2$ as part of a sequence of worlds from $I_1$ to $I_2$ is independent of the worlds where the sequence begins ($\in W_1$) or ends ($\in W_2$).

Since every Kripke model with probabilistic mapping is memoryless, the probability of transitioning from a world $w_x \in W_1$ to a world in a sequence that leads to $W_2$ is independent of the sequence that lead to the world $w_1$.

Therefore, since every transition for every sequence of worlds from $I_1$ to $I_2$ is memoryless, the transition from $I_1$ to $I_2$ must be memoryless as well. If this were not the case, there must be some transition in the sequence of worlds from $I_1$ to $I_2$ that is not memoryless, which contradicts the memorylessness of the Kripke model with probabilistic mapping.

\begin{case}
    The Markov chain is hidden.
\end{case}

If the Markov chain is hidden, there must be information encoded in the worlds that is not captured by the imaging function.
However, the underlying transitions between worlds which are hidden are still obligated to be memoryless.
For an \ac{HMM} only the hidden variables are obligated to be memoryless, therefore, a \ac{HMM} constructed from a Kripke Model with probabilistic mapping and Imaging function is always memoryless.
\end{proof}
...
If no messages are lost, the sequence of worlds that a process goes through between states depends only on the ordering of events in the system, which are equally likely.
