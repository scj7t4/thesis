...
However, in a general omission model, this level of loyalty from processes may be impractical to achieve.
...
We assume that in our system, any beliefs an agent hold stem from information transfer from another agent.
...
Intuitively, one or more worlds will look identical to a process $i$, since the information necessary to differentiate them is not in $i$'s domain.
...
If $\Pr(X_{k+1} | X_k=x_i)=\Pr(O(D_i,X_k+1) | Y_k=O(D_i,x_i))$, $\forall x_i \in X$ and $\forall y_j \in Y$ such that $O(D_i,x_i)=y_j$, then the observations of a process are a Markov chain.
