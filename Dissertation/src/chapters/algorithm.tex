...
As part of the execution model described, execution proceeds in a partially synchronous fashion.
...
% for a third process.
\begin{thm}
In the modified election algorithm a lower priority process cannot construct a non hidden model.
\end{thm}
...
\begin{proof}
Consider a system with three processes: A, B, and C.
Process priorities are assigned such that $A > B > C$.
Next, consider two worlds that produce the same image for process B: the world where C is grouped with A ($w_1$) and the world where C is not grouped with A ($w_2$).
Additionally, the world $w_3$ as one of the outcomes of a round of execution where B is grouped with C.

In the world where A is grouped with C, process B has no chance of grouping with C, as C is already with its ideal coordinator.
Therefore $\Pr(w_1 \rightarrow \w_3) = 0$.

If C is not grouped with A then B can potentially group with process C.
Therefore $\Pr(w_1 \rightarrow \w_3) \neq 0$

Because the state of C is MSDND secure to B each time a round executes, any imaging function for B cannot include information which would differentiate between $w_1$ and $w_2$.
Therefore $Im(w_1, B) = Im(w_2, B)$.
Since $\Pr(w_1 \rightarrow w_3) \neq \Pr(w_1 \rightarrow w_3)$ which violates the memorylessness property.
\end{proof}
...
Let $W_O$ be the set of worlds that describe outcomes of a round of execution in a modeled partially synchronous leader election.
Let $W
\begin{thm}
\end{thm}
