...
\subsection{Lower Priority Process}

A precise accurate model that is time homogeneous cannot be constructed for a process that is not the highest priority.
Based on the presented algorithm, a lower priority process constructing a model has several challenges.
First, if the process is invited to join the group of the higher priority process its model is invalidated.
Secondly, the lower priority process cannot know which processes are already in a group with the higher priority process.

In the execution model we have presented, the inclusion of a process in a higher priority proccess's group is MSDND secure.
...
% for a third process.
\begin{thm}
    In the modified invitation election algorithm only the highest priority process's model can be constructed.
\end{thm}

\begin{proof}
Consider a system state with 3 processes, such that the priority of processes is $A > B > C$.
Suppose process A leads a group with process C.
If B and C have exchanged no messages, they are MSDND secure by Theorem NO MESSAGES.
In this case, B does not know that C is in a group with A because C's state is MSDND secure to B.
B cannot determine the probability that an invitation to C will succeed, because this depends on system state hidden to B.
\end{proof}
...
Therefore, unlike the highest priority process, any other process cannot construct a definitive model.

However, any process in the system equipped with the number of processes and the omission rate, can construct the same model of system as the highest priority process.
