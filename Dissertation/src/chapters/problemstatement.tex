...
Additionally, we hoped to determine the advantages and disadvantages of using a more complicated, reliabile message retransmission protocol over a simplier one without retransmission.

\section{Intial Experiments}

The initial experiments were collected from a non-real time version of the DGI code.
Expiriments measured the time-in-group for various sets of DGI running the leader election algorithm during omission fault conditions.
...
Aftet collecting the results from the intial experiments, we sought to describe the observed behavior through the use of continous-time markov chains.
The behavior of the DGI transition between various states of grouping was calibrated with intial results and applied to other scenarios to validate the results.
This approach had several short comings.
First, for reasons we will demonstrate in subsequent chapters, the leader election algorithm modeled in these chains was not memoryless: the state used in the Markov chain was not sufficent capture the interaction between processes that was occuring.
Secondly, the continous time model could not accurate capture the execution model of the DGI.
...
These results results were not sufficent to accurately describe the behavior of the system during fault conditions.
As a result, we sought to refine the analysis of the model in order to get an accurate portrayl of the behavior of the system during faults.

\section{Remarks}

In order to ensure that critical infstracture safely and reliabily provides services to those that need the services, it is necessary to understand how the infrastructure behaves during faults.
...
We propose knowledge of the correctness of the operation may be more important that the efficency of the operation of that critical infrastructure.
...
By using these models one can analyze where the certainity of the system is placed, as well as ensuring that the algorithms can be modelled without complete and perfect information of the system.
...
With this hardening techinque the algorithms can prevent critical failures which could decrease quality of life or endanger that life itself.
Subsequent chapters show how information flow methods can be used to determine the memorylessness of aspects of a distributed system, how those aspects can be used to contruct a model, and applications of those models.
