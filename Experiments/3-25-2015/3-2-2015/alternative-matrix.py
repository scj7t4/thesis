...
def do_chi_trans(p, observed):
    pre = chain.design(3,p)
    #print "OBSERVED:\n{}".format(json.dumps(jsonify(obv2prob(observed)),indent=2))
    #print "PREDICTION:\n{}".format(json.dumps(jsonify(pre), indent=2))
    return chi_test(pre, observed)

def do_log_steady(p, obv):
    pre = chain.design(4,p)
    T = pykov.Chain(pre)
...
def main():
    ...
    for i in range(5,100,5):
        ...
        print "{}\t{}".format(p,do_chi_trans(p,observed))
