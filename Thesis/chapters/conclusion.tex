...
Given this, we can then consider the time that a process may live for, the time it would take to reconfigure, and the likelyhood of violating the physical invariant into account as part of selecting a real-time schedule.
...
\begin{table}
\caption{Comparsion of two proposed schedules, A and B}
...
Based on the above equation, schedule B should be selected when delta is negative, yielding the invariant for the current schedule x and the set of potential schedules S:
